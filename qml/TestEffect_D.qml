...
Item {
    ...
    ShaderEffect {
        ...
        SequentialAnimation {
            running: true
            loops: -1
            NumberAnimation { target: rootShader; property: "iTime"; from: 0.0; to: 3.1415926 * 2; duration: 5000 }
            ScriptAction { script: { console.log("-----------------------") } }
            NumberAnimation { target: rootShader; property: "iTime"; from: 3.1415926 * 2; to: 0.0; duration: 5000 }
        }
        ...
        fragmentShader: "
            ...
            vec3 background(vec2 pos) {
                ...
                for (int i = 0; i < 3; ++i) {
                    color += mix(texture2D(source1, pos - 0.25 * offset + 0.5),
                                 texture2D(source1, pos - offset),
                                 abs(mod(float(i) * 0.666, 2.0) - 1.0) ).xyz * color * color;
                }
                return color * vec3(0.666, 0.266, 0.0333);
            }
            ...
            void main() {
                ...
                gl_FragColor.rgb = gl_FragColor.rgb * step(0.01, gl_FragColor.a) +  background(0.1 * uv) * step(gl_FragColor.a, 0.01);
            }"
    }
}
